...
import {
  ...
  ADVERSARIAL_SUITE,
} from '../dist/src/cognitive/index.js';
...
describe('SpikeTensor (organoid/MEA)', () => {
  it('should produce a valid spike tensor with stats', () => {
    ...
    const stats = t.stats();
    assert.equal(stats.electrodes, 8);
    assert.equal(stats.timeBins, 64);
    assert.ok(stats.spikeCount <= 8 * 64);
    assert.ok(stats.meanRate >= 0 && stats.meanRate <= 1);
    assert.equal(stats.data, undefined); // stats has no data field
  });
  ...
  it('should enforce refractory period', () => {
    ...
    // no two spikes within <3 bins on same electrode
    const d = t.data;
    for (let e = 0; e < 2; e++) {
      let last = -99;
      for (let b = 0; b < 10; b++) {
  ...
  it('should slice a time window', () => {
    ...
    assert.equal(s.config.timeBins, 8);
    assert.equal(s.config.electrodes, 4);
  });
});
...
describe('ControlLoop (closed-loop robotic control)', () => {
  ...
  it('should return a step result with coherence', () => {
    ...
    assert.equal(r.sensor.length, 4);
...
describe('adversarialStressTest (neural security layers)', () => {
  it('should run the full suite and produce a report', () => {
    ...
    assert.equal(report.attacks.length, ADVERSARIAL_SUITE.length);
    ...
    for (const atk of report.attacks) {
      assert.ok(typeof atk.detected === 'boolean');
  ...
  it('should flag known catastrophic classes as detected', () => {
    ...
    assert.equal(catastrophic.detected, true);
  });
});
